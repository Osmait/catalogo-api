from fastapi import APIRouter, Path, UploadFile,Depends,status
...
def create_product(product: Product) -> dict:
    ...
    return JSONResponse(status_code=status.HTTP_201_CREATED,content=CREATE_MESSEGE)


@product_router.get("/product", tags=["product"], response_model=List[Product], status_code=status.HTTP_200_OK)
def get_products() -> List[Product]:
    
    result =  ProductService().get_movies()
    return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(result))


@product_router.get("/product/{id}", tags=["product"], response_model=Product, status_code=status.HTTP_200_OK)
def get_product_byId(id: int = Path(ge=1)) -> Product:
    
    result = ProductService().get_movie_byId(id)
